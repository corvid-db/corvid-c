 ...
 *
 * The body below is kept deliberately IDENTICAL to upstream's harness
 * (modulo this header) so the two suites are diffable and their
 * pass/fail verdicts comparable: the same fixture grammar, the same
 * dispatch table, the same checks, one OP<TAB>args<TAB>expected line at
 * a time, every line dispatched, every expectation checked.
 *
 * Conventions carried over from upstream (its C-surface radar enforced
 * the first; we keep it so the files stay diffable):
 *   - NO macros that mention `corvid_` and no includes beyond system
 *     headers and "corvid.h".
 *   - Every handle/buffer this harness creates is freed on the path
 *     that created it: the ASan/LSan CI variant must report ZERO
 *     leaks, exercising every handle family's free function plus
 *     corvid_free's buffer domain.
 *
